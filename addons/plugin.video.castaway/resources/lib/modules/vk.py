...
def resolve(url):
    try:
        ...
        url = []
        try: url += [{'quality': '720p', 'url': result['url720']}]
        except: pass
        try: url += [{'quality': '540p', 'url': result['url540']}]
        except: pass
        try: url += [{'quality': '480p', 'url': result['url480']}]
        except: pass
        if not url == []: return url
        try: url += [{'quality': '360p', 'url': result['url360']}]
        except: pass
        if not url == []: return url
        try: url += [{'quality': '240p', 'url': result['url240']}]
        except: pass
