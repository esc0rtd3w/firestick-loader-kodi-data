...
class updater:
    ...
    def check_resolver(self):
        ...
        try:
            r = cache.get(client.request, 1, base64.b64decode('aHR0cHM6Ly9yYXcuZ2l0aHVidXNlcmNvbnRlbnQuY29tL3hpYmFsYmExMC9zY3JpcHQubW9kdWxlLmFkdWx0cmVzb2x2ZXIvbWFzdGVyL2xpYi9hZHVsdHJlc29sdmVyL3Jlc29sdmVyLnB5'))
            if len(r)>1:
                with open(self.resolverFile,'r') as f: compfile = f.read()
                if 'import' in r:
                    if compfile == r: 
                        log_utils.log('Resolver checked and up to date!', log_utils.LOGNOTICE)
                        pass
                    else:
...
class streamer:
    ...
    def youjizz(self, url):
        try:        
            ...
            s = [e.replace('\\','') for e in i if ( '.mp4' in e ) and ( '.m3u8' not in e )]
            self.u = []
            def request(i):
                try:
                    c = client.request(i, output='headers')
            ...
            u = sorted(self.u, key=lambda x: x[1])[::-1]
            u = client.request(u[0][0], output='geturl', referer=url)
            log_utils.log('Returning %s from YouJizz' % str(u), log_utils.LOGNOTICE)
            return u
        except: 
            return 
