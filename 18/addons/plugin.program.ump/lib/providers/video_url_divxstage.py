...
def run(hash,ump,referer=None):
	src=ump.get_page("http://www.cloudtime.to/video/"+hash,"utf-8")
	sk=re.findall('<input type="hidden" name="stepkey" value="(.*?)">',src)[0]
	src=ump.get_page("http://www.cloudtime.to/video/"+hash,"utf-8",data={"stepkey":sk,"submit":"submit"},referer="http://www.cloudtime.to/video/"+hash)
