...
"""
    ...
    Returns the photoshop Movie list-

    <dir>
    <title>photoshop</title>
    <photoshop>movies</photoshop>
    </dir>
...
----------------------------------------------------------
"""
table_id = "appvDeEX2KgNblEuw"
table_name = "photoshop"
...
class photoshop(Plugin):

    name = "photoshop"


    def process_item(self, item_xml):
        if "<photoshop>" in item_xml:
            item = JenItem(item_xml)
            if "movies" in item.get("photoshop", ""):
                result_item = {
                    'label': item["title"],
                    'icon': item.get("thumbnail", addon_icon),
                    'fanart': item.get("fanart", addon_fanart),
                    'mode': "open_photoshop_movies",
                    'url': "",
                    'folder': True,
                    'imdb': "0",
                    'season': "0",
            ...
            elif "genre" in item.get("photoshop_movie", ""):    
                result_item = {
                    'label': item["title"],
                    'icon': item.get("thumbnail", addon_icon),
                    'fanart': item.get("fanart", addon_fanart),
                    'mode': "open_photoshop_genre_movies",
                    'url': item.get("photoshop_movie", ""),
                    'folder': True,
                    'imdb': "0",
                    'season': "0",
                    'episode': "0",
...
@route(mode='open_photoshop_movies')
def open_movies():
    xml = ""
...
@route(mode='open_photoshop_genre_movies',args=["url"])
