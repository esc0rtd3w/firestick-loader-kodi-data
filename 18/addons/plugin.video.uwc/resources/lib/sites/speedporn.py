...
def Main():
    utils.addDir('[COLOR hotpink]Categories[/COLOR]', 'https://speedporn.net/home/', 783, '', '')
...
def List(url):
    try:
        ...
        return None
    match = re.compile('a href="([^"]+)" title="([^"]+)".+?img data-src="([^"]+)"', re.DOTALL | re.IGNORECASE).findall(listhtml)
    for videopage, name, img in match:
        name = utils.cleantext(name)
        utils.addDownLink(name, videopage, 782, img, '')
    try:
        (nextp, npage) = re.compile('href="([^"]+/(\d+)/)">Next', re.DOTALL | re.IGNORECASE).findall(listhtml)[0]
        utils.addDir('Next Page (' + str(npage) + ')', nextp, 781,'')
    except:
        pass
    xbmcplugin.endOfDirectory(utils.addon_handle)
...
def Categories(url):
    ...
    match = re.compile('<a href="(https://speedporn.net/category/[^"]+)">([^<]+)</a>', re.DOTALL | re.IGNORECASE).findall(cathtml)
