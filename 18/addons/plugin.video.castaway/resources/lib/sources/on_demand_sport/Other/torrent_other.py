...
class info():
    def __init__(self):
    	self.mode = 'torrent_other'
        self.name = 'Other sports torrents (full replays)'
...
class main():
	def __init__(self,url = 'http://www.sport-video.org.ua/other.html'):
	...
	def next_page(self):
		try:
			page = re.findall('other(\d+).html',self.url)[0]
			next_page = self.url.replace('other%s.html'%(page),'other%s.html'%(int(page)+1))
		except:
			next_page = self.url.replace('other.html','other1.html')
		return next_page
