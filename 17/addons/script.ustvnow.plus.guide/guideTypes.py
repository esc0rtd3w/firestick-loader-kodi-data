...
ADDON = xbmcaddon.Addon(id='script.ustvnow.plus.guide')
...
class GuideTypes(object):
    ...
    filePath = xbmc.translatePath(os.path.join('special://profile', 'addon_data', 'script.ustvnow.plus.guide', 'guides.txt'))

    def __init__(self):
        try:
            fetcher = FileFetcher('guides.txt', ADDON)
            ...
            xbmc.log('[script.ustvnow.plus.guide] GuideTypes collected: %s' % str(self.guideTypes), xbmc.LOGDEBUG)

            ADDON.setSetting('xmltv.type', str(defaultGuideId))
        except:
            print 'unable to parse guides.txt'
    ...
    def SectionMap(self, section):
        ...
        for option in options:
            try:
                ...
                if dict1[option] == -1:
                    xbmc.log('[script.ustvnow.plus.guide] skip: %s' % option, xbmc.LOGDEBUG)
            except:
    ...
    def getGuideDataItem(self, id, item):
        ...
        try:
            ...
            xbmc.log('[script.ustvnow.plus.guide] DataItem with index %s not found' % item, xbmc.LOGDEBUG)
        return value


    def getGuideById(self, id):
        xbmc.log('[script.ustvnow.plus.guide] Finding Guide with ID: %s' % id, xbmc.LOGDEBUG)
        ret = []
        for guide in self.guideTypes:
            if guide[self.GUIDE_ID] == int(id):
                ret = guide
                xbmc.log('[script.ustvnow.plus.guide] Found Guide with data: %s' % str(guide), xbmc.LOGDEBUG)
        return ret
...
if __name__ == '__main__':
    ...
    if ret >= 0:
        ...
        if xbmc.getCondVisibility('system.platform.android') and int(ver) < 15:
            ...
            xbmc.log('[script.ustvnow.plus.guide] Running on ANDROID with Kodi v%s --> using workaround!' % str(ver), xbmc.LOGDEBUG)
            filePath = xbmc.translatePath(os.path.join('special://profile', 'addon_data', 'script.ustvnow.plus.guide', 'settings.xml'))
