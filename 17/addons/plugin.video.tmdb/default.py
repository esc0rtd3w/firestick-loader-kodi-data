...
def root():
    ...
    if enable_newswin == 'true':
        koding.Add_Dir(name='                                                  [B]LATEST NEWS AND UPDATES[/B]', url='{"my_text":"[B]Latest News And Updates[/B][CR]!!!","my_desc":""}', mode='dialog_example', folder=False, icon=os.path.join(art_path,'icon.png'), fanart=os.path.join(art_path,'fanart.jpg'))
