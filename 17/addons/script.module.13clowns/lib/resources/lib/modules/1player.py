...
class player(xbmc.Player):
    ...
    def keepPlaybackAlive(self):
        ...
        if overlay == '7':
            ...
            while self.isPlayingVideo():
             ...
             try:
                nextUpInfo = self.nextUpSend(self.meta)
                AddonSignals.sendSignal('upnext_data', nextUpInfo, source_id='plugin.video.13clowns')
                AddonSignals.registerSlot('upnextprovider', 'plugin.video.13clowns_play_action', self.nextUpReceive)
            except:
                pass
    ...
    def onPlayBackEnded(self):
        ...
        if control.setting('crefresh') == 'true':
            xbmc.executebuiltin('Container.Refresh')
###UPNEXT
def nextUpReceive(self, data):

        self.stop()
        control.sleep(200)

        meta = data['meta']
        dtime = (datetime.datetime.utcnow() - datetime.timedelta(hours = 5))
        systime = (dtime).strftime('%Y%m%d%H%M%S%f')

        try:
            url = '%s?action=play&title=%s&year=%s&imdb=%s&tvdb=%s&season=%s&episode=%s&tvshowtitle=%s&premiered=%s&meta=%s&t=%s' \
                % (sys.argv[0], urllib.quote_plus(meta['title']), meta['year'], meta['imdb'], meta['tvdb'], meta['season'], meta['episode'], urllib.quote_plus(meta['tvshowtitle']), urllib.quote_plus(meta['premiered']), urllib.quote_plus(json.dumps(meta)), systime)
            control.execute('RunPlugin(%s)' % url)
        except:
            control.infoDialog('Autoplay Failed', time=5000)

    def nextUpSend(self, meta):
        from resources.lib.indexers.episodes import episodes
        ...
        currentEpisode["showtitle"] = meta['tvshowtitle']
        currentEpisode["season"] = meta['season']
        currentEpisode["episode"] = meta['episode']
        currentEpisode["episodeid"] = '%02d.%02d' % (int(meta['season']), int(meta['episode']))
        currentEpisode["tvshowid"] = meta['tvdb']
        currentEpisode["title"] = meta['label']
        currentEpisode["plot"] = meta['plot']
        currentEpisode["playcount"] = ''
        currentEpisode["rating"] = meta['rating']
        currentEpisode["firstaired"] = meta['year']
        currentEpisode["art"]["tvshow.poster"] = meta.get('poster', '')
        currentEpisode["art"]["thumb"] = meta.get('thumb', '')
        currentEpisode["art"]["tvshow.fanart"] = meta.get('fanart', '')
        currentEpisode["art"]["tvshow.landscape"] = ''
        currentEpisode["art"]["tvshow.clearart"] = ''
        currentEpisode["art"]["tvshow.clearlogo"] = ''

        seasonMeta = episodes().get(meta['tvshowtitle'], meta['year'], meta['imdb'], meta['tvdb'], create_directory=False)
        nextEpisodeMeta = [i for i in seasonMeta if i['season'] == str(meta["season"]) and i['episode'] == str(int(meta["episode"])+1) and not i['unaired'] == 'true']
        if not nextEpisodeMeta: nextEpisodeMeta = [i for i in seasonMeta if i['season'] == str(int(meta["season"])+1) and i['episode'] == '1' and not i['unaired'] == 'true']

        try:
            nextEpisodeMeta = nextEpisodeMeta[0]
            nextEpisode["showtitle"] = nextEpisodeMeta['tvshowtitle']
            nextEpisode["season"] = nextEpisodeMeta['season']
            nextEpisode["episode"] = nextEpisodeMeta['episode']
            nextEpisode["episodeid"] = '%02d.%02d' % (int(nextEpisodeMeta['season']), int(nextEpisodeMeta['episode']))
            nextEpisode["tvshowid"] = nextEpisodeMeta['tvdb']
            nextEpisode["title"] = nextEpisodeMeta['label']
            nextEpisode["plot"] = nextEpisodeMeta['plot']
            nextEpisode["playcount"] = ''
            nextEpisode["rating"] = nextEpisodeMeta['rating']
            nextEpisode["firstaired"] = nextEpisodeMeta['year']
            nextEpisode["art"]["tvshow.poster"] = nextEpisodeMeta.get('poster', '')
            nextEpisode["art"]["thumb"] = nextEpisodeMeta.get('thumb', '')
            nextEpisode["art"]["tvshow.fanart"] = nextEpisodeMeta.get('fanart', '')
            nextEpisode["art"]["tvshow.landscape"] = ''
            nextEpisode["art"]["tvshow.clearart"] = ''
            nextEpisode["art"]["tvshow.clearlogo"] = ''

            playInfo["meta"] = nextEpisodeMeta

            nextInfo = {
                "current_episode": currentEpisode,
                "next_episode": nextEpisode,
                "play_info": playInfo,
                "notification_time": int(control.setting('upnexttime'))
                }
        except:
            pass
