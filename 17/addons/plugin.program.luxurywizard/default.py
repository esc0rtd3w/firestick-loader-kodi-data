...
INCLUDECHOC       = wiz.getS('includechoc')
INCLUDEDEATH   = wiz.getS('includedeath')
INCLUDEDEATHRD    = wiz.getS('includedathrd')
INCLUDEPLACE   = wiz.getS('includeplace')
INCLUDENEPTUNE    = wiz.getS('includeneptune')
INCLUDEKNOB   = wiz.getS('includeknob')
INCLUDEDEATH     = wiz.getS('includedeath')
INCLUDEDEATHRD   = wiz.getS('includedeathrd')
...
def index():
	...
	addDir ('Speed Tester' ,'net', icon=ICONSPEED, themeit=THEME1)
...
def maintMenu(view=None):
	...
	if includeall == 'true':
		...
		includechoc = 'true' if INCLUDECHOC     == 'true' else 'false'
		includedeath = 'true' if INCLUDEDEATH == 'true' else 'false'
		includedeathrd = 'true' if INCLUDEDEATHRD  == 'true' else 'false'
		includeplace = 'true' if INCLUDEPLACE == 'true' else 'false'
		includeneptune = 'true' if INCLUDENEPTUNE  == 'true' else 'false'
		includeknob = 'true' if INCLUDEKNOB == 'true' else 'false'
		includedeath = 'true' if INCLUDEDEATH   == 'true' else 'false'
		includedeathrd = 'true' if INCLUDEDEATHRD == 'true' else 'false'
	...
	if includevid == 'true':
		...
		addFile('--- Include Choc: %s' % includechoc.replace('true',on).replace('false',off), 'togglecache', 'includechoc', icon=ICONMAINT, themeit=THEME3)
		addFile('--- Include Death: %s' % includedeath.replace('true',on).replace('false',off), 'togglecache', 'includedeath', icon=ICONMAINT, themeit=THEME3)
		addFile('--- Include Deathrd: %s' % includedeathrd.replace('true',on).replace('false',off), 'togglecache', 'includedeathrd', icon=ICONMAINT, themeit=THEME3)
		addFile('--- Include Place: %s' % includeplace.replace('true',on).replace('false',off), 'togglecache', 'includeplace', icon=ICONMAINT, themeit=THEME3)
		addFile('--- Include Neptune: %s' % includeneptune.replace('true',on).replace('false',off), 'togglecache', 'includeneptune', icon=ICONMAINT, themeit=THEME3)
		addFile('--- Include Knob: %s' % includeknob.replace('true',on).replace('false',off), 'togglecache', 'includesknob', icon=ICONMAINT, themeit=THEME3)
...
def toggleCache(state):
	cachelist = ['includevideo', 'includeall', 'includechoc', 'includedeath', 'includedeathrd', 'includeplace', 'includeneptune', 'includeknob']
	titlelist = ['Include Video Addons', 'Include All Addons', 'Include Choc', 'Include Death', 'Include Deathrd', 'Include Place', 'Include Neptune', 'Include Knob']
...
def net_tools(view=None):
	if HIDESPACERS == 'Yes': addFile(wiz.sep('Speed Tester'), '', themeit=THEME3)
