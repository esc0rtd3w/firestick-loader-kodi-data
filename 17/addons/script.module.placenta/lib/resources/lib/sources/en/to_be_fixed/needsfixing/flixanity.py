...
class source:
    ...
    def searchShow(self, title, season, episode, aliases, headers):
        try:
            url = '%s/series/%s/season/%01d/episode/%01d' % (self.base_link, cleantitle.geturl(title), int(season), int(episode))
            url = client.request(url, headers=headers, output='geturl', timeout='10')
            if url == None:
                for alias in aliases:
                    url = '%s/series/%s/season/%01d/episode/%01d' % (self.base_link, cleantitle.geturl(alias['title']), int(season), int(episode))
                    url = client.request(url, headers=headers,output='geturl', timeout='10')
