"""
    ...
    Returns the Duke Movie list-

    <dir>
    <title>The Duke Movie List</title>
    <duke>all</duke>
	<thumbnail>https://pbs.twimg.com/profile_images/1004442158248554497/k4iRjaEN_400x400.jpg</thumbnail>
	<fanart>http://www.fullhdwpp.com/wp-content/uploads/John-Wayne-quote_www.fullhdwpp.com_.jpg</fanart>
    </dir>
...
class The_Duke(Plugin):
    ...
    def process_item(self, item_xml):
        if "<duke>" in item_xml:
            ...
            if "all" in item.get("duke", ""):
                result_item = {
                    'label': item["title"],
                    'icon': item.get("thumbnail", addon_icon),
                    'fanart': item.get("fanart", addon_fanart),
                    'mode': "open_the_duke_movies",
                    'url': "",
                    'folder': True,
                    'imdb': "0",
                    'season': "0",
...
@route(mode='open_the_duke_movies')
def open_movies():
    xml = ""
