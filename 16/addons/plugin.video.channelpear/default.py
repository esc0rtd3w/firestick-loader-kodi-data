...
import urllib,urllib2,datetime,re,os,traceback,cookielib
import checkaddon,time,requests,base64
...
exec("import re;import base64");exec((lambda p,y:(lambda o,b,f:re.sub(o,b,f))(r"([0-9a-f]+)",lambda m:p(m,y),OU812("NCA9IFt7IjE1IjogIjA9PSIuMignMScpLjIoJzEnKS4yKCcxJykuMignMScpLCAiYiI6ICJkOi8vMy4xOC9jLzkuMTYiLCAiMTAiOiAiMTQgOCJ9XQphID0gNAoKCjEzIDcoNik6CgkxYSBlID09ICcxMic6CgkJMTEuMTcoIltmLjMtJTE5XTogJTE5IiAlKDUsIDYpKQ==")))(lambda a,b:b[int("0x"+a.group(1),16)],"V1ZWb1UwMUhUa1ZpTTFwTlRUQktNbGRyWkd0a2JIQkVUbGhhWVdKV2NEWlpWV00xWlZad1dWRnRhR3BOTVVwelYxY3hjMlJWZUhSVWJscHBWWHBzY1Zrd1pGZGhSMDV3VDFkd2FsSXhXbTlaTW1zeFRVZFdTVlZVTUQwPQ|base64|decode|channelpear|sourceURL|addon_version|string|addon_log|Channels|kiwi_new|SOURCES|fanart|images|https|debug|addon|title|xbmc|true|def|All|url|jpg|log|com|s|if".split("|")))
...
def makeRequest(url, headers=None):
        try:
            ...
            if hasattr(e, 'code'):
                ...
                xbmc.executebuiltin("XBMC.Notification(ChannelPEAR,We failed with error code - "+str(e.code)+",10000,"+icon+")")
            elif hasattr(e, 'reason'):
                addon_log('We failed to reach a server.')
                addon_log('Reason: %s' %e.reason)
                xbmc.executebuiltin("XBMC.Notification(ChannelPEAR,We failed to reach a server. - "+str(e.reason)+",10000,"+icon+")")
...
def addSource(url=None):
        ...
        xbmc.executebuiltin("XBMC.Notification(ChannelPEAR,New source added.,5000,"+icon+")")
        if not url is None:
...
def createM3uForDash(url,useragent=None):
    ...
    source_file2 = os.path.join(profile, 'testfile.m3u')
    str+='\n'
    SaveToFile(source_file2,str)
...
def urlsolver(url):
    ...
    if host:
        ...
        xbmc.executebuiltin("XBMC.Notification(ChannelPEAR,Urlresolver donot support this domain. - ,5000)")
    return resolver
...
def download_file(name, url):
        if addon.getSetting('save_location') == "":
            xbmc.executebuiltin("XBMC.Notification('ChannelPEAR','Choose a location to save files.',15000,"+icon+")")
            addon.openSettings()
        params = {'url': url, 'download_path': addon.getSetting('save_location')}
        downloader.download(name, params)
        dialog = xbmcgui.Dialog()
        ret = dialog.yesno('ChannelPEAR', 'Do you want to add this file as a source?')
        if ret:
...
def addDir(name,url,mode,iconimage,fanart,description,genre,date,credits,showcontext=False,regexs=None,reg_url=None,allinfo={}):
        ...
        if showcontext:
            ...
            if showcontext == 'source':
                ...
                contextMenu.append(('Remove from ChannelPEAR Favorites','XBMC.RunPlugin(%s?mode=6&name=%s)'
            ...
            if not name in FAV:
                contextMenu.append(('Add to ChannelPEAR Favorites','XBMC.RunPlugin(%s?mode=5&name=%s&url=%s&iconimage=%s&fanart=%s&fav_mode=%s)'
...
def addLink(url,name,iconimage,fanart,description,genre,date,showcontext,playlist,regexs,total,setCookie="",allinfo={}):
        ...
        if showcontext:
            ...
            if showcontext == 'fav':
                contextMenu.append(
                    ('Remove from ChannelPEAR Favorites','XBMC.RunPlugin(%s?mode=6&name=%s)'
                     %(sys.argv[0], urllib.quote_plus(name)))
                     )
                ...
                contextMenu.append(('Add to ChannelPEAR Favorites','XBMC.RunPlugin(%s)' %fav_params))
            liz.addContextMenuItems(contextMenu)
...
if mode==None:
    ...
    if url:
        ...
        xbmc.executebuiltin("XBMC.Notification(ChannelPEAR,Failed to extract regex. - "+"this"+",4000,"+icon+")")
elif mode==18:
    addon_log("youtubedl")
    try:
        import youtubedl
    except Exception:
        xbmc.executebuiltin("XBMC.Notification(ChannelPEAR,Please [COLOR yellow]install Youtube-dl[/COLOR] module ,10000,"")")
