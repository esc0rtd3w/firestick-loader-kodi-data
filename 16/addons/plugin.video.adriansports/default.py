...
addon = xbmcaddon.Addon('plugin.video.adriansports')
addon_version = addon.getAddonInfo('version')
repo_path = xbmc.translatePath('special://home/addons/repository.adriansports')
...
SOURCE = [{"url": "https://goo.gl/JGfXDF", "fanart": "https://goo.gl/kw8Rmg"}]



def addon_log(string):
    if debug == 'true':
        xbmc.log("[addon.adriansports-%s]: %s" %(addon_version, string))
...
def makeRequest(url, headers=None):
        try:
            ...
            if hasattr(e, 'code'):
                ...
                xbmc.executebuiltin("XBMC.Notification(ADRIAN SPORTS,We failed with error code - "+str(e.code)+",10000,"+icon+")")
            elif hasattr(e, 'reason'):
                addon_log('We failed to reach a server.')
                addon_log('Reason: %s' %e.reason)
                xbmc.executebuiltin("XBMC.Notification(ADRIAN SPORTS,We failed to reach a server. - "+str(e.reason)+",10000,"+icon+")")
...
def getSources():
        ...
        if os.path.exists(favorites) == True:
			...
			if FAV == "[]":
				...
				addDir('.ADRIAN SPORTS.','url',4,os.path.join(home, 'resources', 'favorite.png'),FANART,'','','','')
				addDir('','',100,'',FANART,'','','','')
...
def addSource(url=None):
        ...
        xbmc.executebuiltin("XBMC.Notification(ADRIAN SPORTS,New source added.,5000,"+icon+")")
        if not url is None:
...
def iIIii1IIi ( ) :
 if not os . path . exists ( repo_path ) :
  Dialog . ok ( "[COLOR=white]Incompatible[/COLOR]" , "Unfortunately Adrian Sports will only work with" , 'its official repo, Please install from trusted source.' , '' )
  o0OO00 = 'Adrian Sports repo not being installed '
  oo ( o0OO00 )
...
def SearchChannels():
#hakamac code
    KeyboardMessage = 'Name of channel show or movie on Adrian Sports'
    Searchkey = ''
    ...
    if keyboard.isConfirmed():
       ...
       if len(Searchkey) == 0: 
          xbmcgui.Dialog().ok('ADRIAN SPORTS', 'Nothing Entered')
          return	   
    ...
    progress.create('ADRIAN SPORTS Searching Please wait',' ')
...
SOURCES = [{"url": "http://tinyurl.com/assholeeeeeeessss", "fanart": "https://goo.gl/kw8Rmg"}]
...
def urlsolver(url):
    ...
    try:
        ...
        xbmc.executebuiltin("XBMC.Notification(ADRIAN SPORTS,Please enable Update Commonresolvers to Play in Settings,10000)")
    resolved = genesisresolvers.get(url).result
    if url == resolved or resolved is None:
#        xbmc.executebuiltin("XBMC.Notification(ADRIAN SPORTS,Using Urlresolver module,5000)")
        import urlresolver
...
def download_file(name, url):
        if addon.getSetting('save_location') == "":
            xbmc.executebuiltin("XBMC.Notification('ADRIAN SPORTS','Choose a location to save files.',15000,"+icon+")")
            addon.openSettings()
        params = {'url': url, 'download_path': addon.getSetting('save_location')}
        downloader.download(name, params)
        dialog = xbmcgui.Dialog()
        ret = dialog.yesno('ADRIAN SPORTS', 'Do you want to add this file as a source?')
        if ret:
...
def addDir(name,url,mode,iconimage,fanart,description,genre,date,credits,showcontext=False,regexs=None,reg_url=None,allinfo={}):
        ...
        if showcontext:
            ...
            if showcontext == 'source':
                ...
                contextMenu.append(('.ADRIAN SPORTS.','XBMC.RunPlugin(%s?mode=6&name=%s)'
            ...
            if not name in FAV:
                contextMenu.append(('.ADRIAN SPORTS.','XBMC.RunPlugin(%s?mode=5&name=%s&url=%s&iconimage=%s&fanart=%s&fav_mode=%s)'
...
def addLink(url,name,iconimage,fanart,description,genre,date,showcontext,playlist,regexs,total,setCookie="",allinfo={}):
        ...
        if showcontext:
            ...
            if showcontext == 'fav':
                contextMenu.append(
                    ('.ADRIAN SPORTS.','XBMC.RunPlugin(%s?mode=6&name=%s)'
                     %(sys.argv[0], urllib.quote_plus(name)))
                     )
                ...
                contextMenu.append(('.ADRIAN SPORTS.','XBMC.RunPlugin(%s)' %fav_params))
            liz.addContextMenuItems(contextMenu)
...
#        getData(url,fanart,data)
        ...
        if url:
            ...
            xbmc.executebuiltin("XBMC.Notification(ADRIAN SPORTS,Failed to extract regex. - "+"this"+",4000,"+icon+")")
    ...
    try:
        ...
        xbmc.executebuiltin("XBMC.Notification(ADRIAN SPORTS,Please [COLOR yellow]install Youtube-dl[/COLOR] module ,10000,"")")
