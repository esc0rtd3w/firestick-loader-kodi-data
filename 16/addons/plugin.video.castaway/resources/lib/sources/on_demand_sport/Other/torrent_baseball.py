...
class info():
    def __init__(self):
    	self.mode = 'torrent_baseball'
        self.name = 'Baseball torrents (full replays)'
...
class main():
	def __init__(self,url = 'http://www.sport-video.org.ua/baseball.html'):
	...
	def next_page(self):
		try:
			page = re.findall('baseball(\d+).html',self.url)[0]
			next_page = self.url.replace('baseball%s.html'%(page),'baseball%s.html'%(int(page)+1))
		except:
			next_page = self.url.replace('baseball.html','baseball1.html')
		return next_page
