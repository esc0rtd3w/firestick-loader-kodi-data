...
def done(title, dest, downloaded):
    ...
    text = xbmcgui.Window(10000).getProperty('WC-DOWNLOADED')
    ...
    xbmcgui.Window(10000).setProperty('WC-DOWNLOADED', text)

    if (not downloaded) or (not playing):         
        xbmcgui.Dialog().ok(title, text)
        xbmcgui.Window(10000).clearProperty('WC-DOWNLOADED')
...
def doDownload(url, dest, title, referer='', agent='', cookie='', quiet=False):
    ...
    while True:
        ...
        if percent >= notify:
            ...
            log('Download percent : %s %s %dMB downloaded : %sMB File Size : %sMB' % (str(percent)+'%', dest, mb, downloaded / 1000000, content / 1000000))
