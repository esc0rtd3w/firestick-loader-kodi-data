    ...
    try:
        ...
        for page in range( start, end):
                if ( page != start):
                        ...
                        html = net.http_GET(pageUrl).content                      
                match = re.compile('<div id="mt.+?" class="item">\s*?<a href="http://hdvid.xyz/(.+?)">\s*?<div class="image">\s*?<img src="(.+?)" alt=".+?" width="100%" height="100%" />', re.DOTALL).findall(html)
                for movieUrl, img in match:
                        addon.add_directory({'mode': 'GetLinks', 'section': section, 'url': 'http://hdvid.xyz/' + movieUrl, 'img': img , 'text': movieUrl.replace('.html', '').replace('-', ' ') }, {'title':  movieUrl.replace('.html', '').replace('-', ' ')}, img= img, fanart= 'http://images.forwallpaper.com/files/thumbs/preview/64/646017__cinema_p.jpg')      
...
def GetLinks(section, url, img, text):
        ...
        match2 = re.compile('<span class="dato"><a href="http://www.imdb.com/title/.+?" rel="nofollow" target="_blank">(.+?):</a> <b>(.+?)</b> <b>N/A votes</b></span>').findall(content)
...
def MainMenu():   
        ...
        addon.add_directory({'mode': 'GenreMenu'}, {'title':  '[B]Movies by genre[/B]'}, img=IconPath + 'icon.png', fanart=FanartPath + 'fanart.jpg')
        addon.add_directory({'mode': 'YearMenu'}, {'title':  '[B]Movies by year[/B]'}, img=IconPath + 'icon.png', fanart=FanartPath + 'fanart.jpg')

        addon.add_directory({'mode': 'GetSearchQuery'},  {'title':  '[COLOR green]Search[/COLOR]'}, img=IconPath + 'search11.png', fanart=FanartPath + 'fanart.jpg')
        addon.add_directory({'mode': 'ResolverSettings'}, {'title':  '[COLOR red]Resolver Settings[/COLOR]'}, img=IconPath + 'resolvere.png', fanart=FanartPath + 'fanart.jpg')
