'''
    ...
    Copyright (C) 2013-14 Sean Poyser (seanpoyser@gmail.com)
...
def download(url, dest, title=None, referrer=None, handle=None):
    if handle is not None:
        liz = xbmcgui.ListItem('')
        liz.setPath(url)
        xbmcplugin.setResolvedUrl(int(handle), True, liz)
        xbmc.Player().stop()
